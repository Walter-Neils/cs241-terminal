...
void vector_ensure_space_for(vector *target, size_t desired_element_count) {
  ...
  void* new_allocation = realloc(target->data, desired_element_count * target->element_size);
...
void vector_push_back(vector *target, void *element) {
  ...
  void* destination = vector_get_element_at_index(target, target->current_used_slots);
...
bool vector_iterator_move_next(vector_iterator *iterator) {
  ...
  return iterator->current_position < iterator->target_vector->current_used_slots && iterator->current_position >= 0;
}

void* vector_iterator_current_element(vector_iterator* iterator) {
  return vector_get_element_at_index(iterator->target_vector, iterator->current_position);
}
