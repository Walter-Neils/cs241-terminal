...
bool is_key_match(string key, string target) {
  ...
  if(target_len <= key_len + 1) return false; // Key length plus 1 for the equal sign
...
string environment_get_variable_value(string key) {
  ...
  while(vector_iterator_move_next(&iterator)) {
    ...
    if(is_key_match(key, target)) {
      ...
      return target + key_length + 1; // Length of key + 1 (length of equal sign)
    }
  }
  return NULL;
}
...
void environment_set_variable(string key, string value) {
  ...
  string key_value_pair = malloc(result_length); // Key length, value length, equal sign, and null terminator
...
vector_iterator iterator;
  ...
  while(vector_iterator_move_next(&iterator)) {
    ...
    if(kv_seperator == NULL) {
      perror("[NON FATAL] found incorrectly formatted environment variable kvp\n");
      continue;
    }
...
string environment_raw_key_to_varref_format(string key) {
  size_t key_length = strlen(key);
  size_t result_length = key_length + 1 + 1; // Dollar sign, key length, null terminator
