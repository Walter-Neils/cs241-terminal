...
string *
subprocess_string_vector_to_exec_compatible_memory_region(vector *target) {
  size_t required_region_size =
      (target->element_size *
       (target->current_used_slots +
        1)); // We need an additional slot for the null terminator
  ...
  if (region[target->current_used_slots] != NULL) {
    perror("subprocess_string_vector_to_exec_compatible_memory_region: null "
           "terminator was overwritten");
    exit(EXIT_FAILURE);
  }
...
subprocess_create_result subprocess_run(string filename, vector *argv,
                                        vector *environment) {
  subprocess_create_result result;
  shared_memory_initialize(&result.subprocess_shared_memory_container,
                           sizeof(typeof(*result.subprocess)));
  ...
  if (FORK_IS_PARENT(forkval)) {
    ...
    string *argv_region =
        subprocess_string_vector_to_exec_compatible_memory_region(argv);
    string *environment_region =
        subprocess_string_vector_to_exec_compatible_memory_region(environment);
    ...
    if (FORK_IS_PARENT(subforkval)) {
      ...
      // overwrites the TEXT, DATA, BSS, and STACK of our current process
      execve(filename, argv_region, environment_region); // Does not return
      // execvp(filename, argv_region);
